...
- **`overflow:hidden` on body does NOT block Android Chrome window scroll** — Body overflow only prevents the body element from scrolling. Android Chrome triggers window-level scroll from touch chaining. Fix: use `fixed inset-0 overflow-hidden` on the app shell div — takes body out of document flow so `window.scrollY` cannot move (date: 2026-05-31)
- **Routine step trackerId hallucination** — Gemini sometimes copies the previous step's tracker UUID but writes the new step's `trackerName`. Fix: (1) inject explicit `fld_*` field IDs + labels into the JSON format template in `buildRoutineSystemPrompt`; (2) server-side correction in `buildSanitizedActions` — if `trackerName` matches current step but `trackerId` doesn't, override the UUID (date: 2026-05-31)
- **OAuth callback: `cookies()` from `next/headers` doesn't attach to `NextResponse.redirect()`** — Session cookies set via `cookieStore.set()` are on a phantom response; the redirect response goes out bare. Fix: use `request.cookies.getAll()` for reads, collect `pendingCookies[]` in `setAll`, then `response.cookies.set()` on the `NextResponse.redirect` (date: 2026-05-31)
- **`ReadableStream.start()` nested functions change TypeScript inference** — Moving a helper function (e.g. `buildSanitizedActions`) inside an async `start(controller)` closure causes spread-return types to not satisfy the declared return type. Fix: add explicit `as AnyActionCard` cast on the spread return object (date: 2026-05-31)
- **ActionCard duration display: use `fieldDefinitions[key].type === 'duration'` not `unit === 'hrs'`** — Duration fields store raw seconds; check type not unit string to convert to HH:MM:SS (date: 2026-06-01)
- **Routine auto-advance setTimeout captures stale `currentSessionId`** — On first-session messages `currentSessionId` may be `'new'` in the closure. Fix: capture `finalSessionId` before the timeout and pass as `sessionIdOverride` to `handleSendSilent` (date: 2026-06-01)
- **`markDayStarted` fire-and-forget orphans day_state when stream drops** — Guard must check `active_routine_id` (not `day_started_at`) to allow re-trigger on orphaned state (date: 2026-06-01)
- **Routine auto-advance stale closure blocks step 2+** — `setTimeout` captures `handleSendSilent` from the render where `isLoading=true`; the stale function immediately returns. Fix: store `handleSendSilent` in `handleSendSilentRef` and update ref on every render; call `handleSendSilentRef.current?.()` from the timer (date: 2026-06-04)
- **Routine step UUID drift breaks `hasLoggedCurrentStep`** — `buildSanitizedActions` name-corrects action's `trackerId` to DB UUID; if routine step stored a stale UUID, comparison fails. Fix: also check `a.trackerName === currentStep.trackerName` as fallback; add name-lookup fallback to all tracker lookups in `buildRoutineSystemPrompt` (date: 2026-06-04)
- **Agent Forge logs to previous day — root cause unresolved** — Active agents (Agent Forge) only log to previous day; regular chat and routines unaffected. Blunt `loggingDate = today` fix was attempted but broke cross-midnight routine sessions (reverted). Investigate `activeAgent` branch in `route.ts` — how `loggingDate` flows into agent system prompt vs `buildRoutineSystemPrompt`. Do NOT use `= today` as the fix (date: 2026-06-10)
