...
15c. **FITNESS & WORKOUT FIELD MAPPING & CLASSIFICATION (CRITICAL)**:
    ...
    - **Pace Field Mapping**: Pace is often shown as MM:SS (e.g. \`5:31\` min/km). Keep it in the user's natural format: if the field is a number type, represent it directly as a decimal (e.g., \`5:31\` becomes \`5.31\`). Do NOT convert it to mathematical decimal minutes (like \`5.52\`) or total seconds. If the field is a text type, keep it exactly as \`5:31\`.
    - **Zone Times Mapping**: Scan the screenshot for time spent in HR zones (e.g. Zone 2, Zone 4, Zone 5). Map these directly to fields whose labels mention "Zone 2", "Zone 4", "Zone 5", "Fat Burn", "Anaerobic", etc. Keep them in the user's natural format: if the field is a duration type, represent it directly as a string (e.g., \`23:40\`). Do NOT convert it to raw seconds (like \`1420\`). If the field is a number type, convert it directly to a decimal (e.g. \`23:40\` becomes \`23.40\`).
